...
| `STUDENT_RAG_FAKE_MODE` | `grounded` (default) or `invalid_citation` failure injection |
...
contract response. The route delegates to `RagApplicationService`; it never
re-implements prompting, retrieval, or citation logic.
