...
export interface AdminAppDependencies {
  ...
  repos: Repositories;
  ai: BookCoreContext["ai"];
  aiGateway: import("@ai-smartbook/ai").AiGateway;
  gatewayConfig: import("@ai-smartbook/ai").GatewayEnvConfig;
  guestAskIpHmacSecret: string;
  guestAskRetentionDays: number;
  analytics: ReturnType<typeof makeAnalyticsService>;
  evaluationService: ReturnType<typeof makeEvaluationService>;
  liveEvaluationService: ReturnType<typeof makeLiveEvaluationService>;
  evaluationGovernanceService: ReturnType<typeof makeEvaluationGovernanceService>;
  pilotService: ReturnType<typeof makePilotService>;
  uploadRoot: string;
  appearanceUploadDir: string;
}

export function createAdminApp(dependencies: AdminAppDependencies): express.Express {
  const {
    env,
    repos,
    ai,
    aiGateway,
    guestAskIpHmacSecret,
    guestAskRetentionDays,
    analytics,
    evaluationService,
  ...
  try {
    ...
    if (deleted > 0) console.log(`[guest-ask] startup cleanup removed ${deleted} expired answer(s)`);
  } catch (err) {
    console.warn("[guest-ask] startup cleanup failed", err instanceof Error ? err.message : err);
  }
  if (env.AI_EVALUATION_SCHEDULER_ENABLED === "true") {
...
const APPEARANCE_UPLOAD_DIR = appearanceUploadDir;
...
app.get("/health/ready", (_req, res) => {
  try {
    // A read-only repository query proves that migrations and the configured
    // database handle are usable without exposing any application data.
    repos.aiProviders.listConfigs();
