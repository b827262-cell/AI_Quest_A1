...
describe("QM Status Frontend Constraints", () => {
  it("Browser/Server boundary checks", () => {
    const pageSource = readFileSync(resolve(srcDir, "pages/QmStatusPage.tsx"), "utf8");
  ...
  it("Sidebar registration", () => {
    const sidebarSource = readFileSync(resolve(srcDir, "components/admin/AdminSidebar.tsx"), "utf8");
  ...
  it("API client registration", () => {
    const apiSource = readFileSync(resolve(srcDir, "api.ts"), "utf8");
