...
export default function Home() {
  ...
  async function submit(event?: FormEvent<HTMLFormElement>) {
    ...
    setAnswer(""); setError(""); setAiStatus(""); setDownloadProgress(null); setNeedsGesture(false);
    if (model !== "Auto") { setError("公開體驗目前僅支援 Auto（Chrome 內建 AI）回答。"); return; }
    setIsLoading(true);
    try {
      const finalAnswer = await autoSubmitter.current.submit(prompt, (chunk) => setAnswer((current) => current + chunk), (status, progress) => { setAiStatus(status); setDownloadProgress(typeof progress === "number" ? progress : null); setNeedsGesture(status === "local model requires user activation"); });
      setAnswer(finalAnswer);
    } catch (reason) { setError(reason instanceof Error ? reason.message : "目前無法取得 AI 回答，請稍後再試。"); }
  ...
  return <main className="v2-page">
    ...
    <section className="v2-auto shell" id="auto-answer" aria-labelledby="auto-heading"><div><p className="v2-eyebrow"><span />Chrome Built-in AI</p><h2 id="auto-heading">直接在這裡，開始你的問題。</h2><p>公開體驗使用此裝置可用的 Chrome 內建 AI 或本機 fallback，不會將題目送往雲端 AI 服務。</p></div><form className="v2-auto-form" onSubmit={submit}><label htmlFor="auto-question">輸入你的問題</label><textarea id="auto-question" value={question} onChange={(event) => setQuestion(event.target.value)} placeholder="例如：請解釋這個段落的重點……" disabled={isLoading} /><div className="v2-auto-controls"><select value={model} onChange={(event) => setModel(event.target.value)} aria-label="AI 模型" disabled={isLoading}><option>Auto</option><option>OpenAI</option><option>Gemini</option><option>Kimi</option><option>Qwen</option></select><select value={mode} onChange={(event) => setMode(event.target.value)} aria-label="解題模式" disabled={isLoading}><option>自動判斷</option><option>程式設計</option><option>數學解題</option><option>文科問答</option><option>資通安全</option><option>教材問答</option></select><button className="v2-button v2-button-primary" type="submit" disabled={!question.trim() || isLoading}>{isLoading ? "處理中…" : "送出問題"}</button></div><div className="v2-quick-modes" aria-label="快速題型">{quickModes.map((item) => <button key={item} type="button" aria-pressed={mode === item} onClick={() => setMode(item)} disabled={isLoading}>{item}</button>)}</div>{isLoading && <p className="v2-auto-status" role="status">{statusText(aiStatus, downloadProgress)}</p>}{(aiStatus === "native model download" || aiStatus === "local model download") && isLoading && <button className="v2-gesture-button" type="button" onClick={() => autoSubmitter.current.cancel()}>取消下載</button>}{error && <p className="v2-auto-status" role="alert">{aiStatus ? `[${aiStatus}] ` : ""}{error}</p>}{needsGesture && !isLoading && <button className="v2-gesture-button" type="button" onClick={() => { void submit(); }}>開始下載本機模型</button>}{answer && <section className="v2-auto-answer" aria-label="本機 AI 回答"><p role="status">本機 AI 回答</p><pre>{answer}</pre></section>}</form></section>
