...
export default function Home() {
  ...
  const [downloadBytes, setDownloadBytes] = useState<{ received: number; total: number } | null>(null);
  ...
  // One consent dialog owns exactly one Google handoff budget. Kept in a ref so
  // a double click on 「同意並在新分頁開啟」 hits an already-consumed guard
  // instead of opening a second tab.
  const consentGuardRef = useRef<ReturnType<typeof createAutoFallbackGuard> | null>(null);
  ...
  function retryDownload() {
    ...
    // state from the render it was created in. requestSubmit() on the live
    // form triggers the CURRENT render's onSubmit instead.
    const trySubmit = () => {
  ...
  function handleOtherHandoff(originalQuestion: string) {
    const prompt = originalQuestion.trim();
    if (!prompt) return;
    const url = buildGoogleAiModeUrl(prompt);
    const guard = consentGuardRef.current ?? createAutoFallbackGuard();
    consentGuardRef.current = guard;
    switch (openConsentHandoffTab(url, guard, typeof window === "undefined" ? null : window)) {
      case "opened":
        setBackupGoogleUrl("");
        setError("已在新分頁開啟 Google AI 解答，本頁保持不變。本站不會讀取或回填 Google 的回答。");
        break;
      case "blocked":
        setBackupGoogleUrl(url);
        setError("瀏覽器阻擋了自動開新分頁。請手動點擊下方連結自行開啟（本站不會離開本頁，也不會讀取或回填 Google 的回答）。");
        break;
      default:
        setError("這個問題已經開啟過一次 Google AI 解答。請回到已開啟的分頁，或重新送出另一個問題。");
    }
  }
...
function statusText(status: AutoAiStatus | "", progress: number | null, bytes?: { received: number; total: number } | null) {
  const mb = (n: number) => `${(n / (1024 * 1024)).toFixed(1)}MB`;
  const bytesInfo = bytes && bytes.total > 0 ? ` · 已收 ${mb(bytes.received)}／${mb(bytes.total)}` : "";
  const percent = progress === null ? "" : ` (${Math.round(progress * 100)}%)`;
  if (status === "native ready") return "狀態：native ready（Chrome 內建 AI 已就緒）";
  if (status === "native model download") return `狀態：native model download（下載 Chrome 內建模型中${percent}${bytesInfo}）`;
  ...
  if (status === "local model download") return `狀態：local model download（下載本機模型約 618MB 中${percent}${bytesInfo}）`;
  if (status === "local model ready") return "狀態：local model ready（本機模型就緒，開始生成回答）";
